...
class App(tk.Tk):
    ...
    def director_submit(self):
        input: str = self.manager_query_text.get("1.0", "end-1c")
        self.manager_query_response["text"] = self.database.execute_director_query(input)

    def audience_submit(self):
        input: str = self.manager_query_text.get("1.0", "end-1c")
        self.manager_query_response["text"] = self.database.execute_audience_query(input)
