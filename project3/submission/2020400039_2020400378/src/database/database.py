...
class Database:
    ...
    def director_login(self, username: str, password: str) -> bool:
        try:
            ...
            self.cursor.execute(f"SELECT * FROM director, user WHERE director.username = '{username}' AND \
                director.password = '{password}' AND director.username = user.username")
            result = self.cursor.fetchone()
            if result:
                print("Login Successful")
    ...
    def audience_login(self, username: str, password: str) -> bool:
        try:
            ...
            self.cursor.execute(f"SELECT * FROM audience, user WHERE audience.username = '{username}' AND \
                audience.password = '{password}' AND audience.username = user.username")
            result = self.cursor.fetchone()
            if result:
                print("Login Successful")
    ...
    def read_director_movies(self, username: str) -> str:
        try:
            ...
            movies = "(movie-id, movie-name, year, duration, genre, language, country, platform-id)\n"
    ...
    def create_movie(self, movie_id: str, movie_name: str, theater_id: str, time_slot: str) -> str:
        try:
            ...
            if self.cursor.rowcount == 0:
                self.cursor.execute(f"INSERT INTO movie (id, name, director_username) VALUES ({movie_id}, '{movie_name}', '{self.director}')")
            self.cursor.execute(f"INSERT INTO movie_session (movie_id, theater_id, time_slot) VALUES ({movie_id}, {theater_id}, '{time_slot}')")
            self.connection.commit()
    ...
    def update_predecessor(self, movie_id: str, predecessor_id: list[str]) -> str:
        try:
            ...
            for id in predecessor_id:
                self.cursor.execute(f"INSERT INTO movie_predecessor (movie_id, predecessor_id) VALUES ({movie_id}, {id})")
            self.connection.commit()
    ...
    def read_movies(self) -> str:
        try:
            self.cursor = self.connection.cursor(buffered=True)
            self.cursor.execute(f"USE {self.default_schema}")
            self.cursor.execute(f"SELECT m.id, m.name, ms.theater_id, ms.time_slot FROM movie m, movie_session ms WHERE m.id = ms.movie_id AND m.director_username = '{self.director}' ORDER BY m.id")
            if self.cursor.rowcount == 0:
                return "No movies found"
            result = self.cursor.fetchall()
            ...
            for row in result:
                movie_list.append(f"{row[0]} {row[1]} {row[2]} {row[3]}")
            return "\n".join(movie_list)
    ...
    def read_audiences(self, movie_id: str) -> str:
        try:
            self.cursor = self.connection.cursor(buffered=True)
            self.cursor.execute(f"USE {self.default_schema}")
            self.cursor.execute(f"SELECT a.username, a.name, a.surname FROM audience a, movie_audience ma WHERE a.username = ma.audience_username AND ma.movie_id = {movie_id}")
            if self.cursor.rowcount == 0:
    ...
    def execute_director_query(self, query: str):
        tokens = [token.lower() if i < 2 else token for i, token in enumerate(query.split())]
        # check if the query is valid
        if len(tokens) < 2:
            return "Invalid query"
    ...
    def create_ticket(self, session_id: str):
        try:
            self.cursor = self.connection.cursor(buffered=True)
            self.cursor.execute(f"USE {self.default_schema}")
            self.cursor.execute(f"SELECT m.id, m.name, ms.theater_id, ms.time_slot FROM movie m, movie_session ms WHERE m.id = ms.movie_id AND ms.id = {session_id}")
            if self.cursor.rowcount == 0:
            ...
            if self.cursor.rowcount != 0:
                predecessors = self.cursor.fetchall()
                for predecessor in predecessors:
                    self.cursor.execute(f"SELECT * FROM movie_session ms, movie_audience ma WHERE ms.id = ma.movie_session_id AND ms.movie_id = {predecessor[0]} AND ma.audience_username = '{self.audience}'")
                    if self.cursor.rowcount == 0:
                        return "You need to watch the predecessor movies first"
            self.cursor.execute(f"SELECT COUNT(*) FROM movie_session ms, movie_audience ma WHERE ms.id = ma.movie_session_id AND ms.id = {session_id}")
            self.cursor.execute(f"SELECT capacity FROM theater WHERE id = {theater_id}")
            theater_capacity = self.cursor.fetchone()[0]
            if self.cursor.fetchone()[0] >= theater_capacity:
                return "The theater is full"
            self.cursor.execute(f"INSERT INTO movie_audience VALUES ('{self.audience}', {session_id})")
            self.connection.commit()
    ...
    def read_movies_audience(self):
        try :
            self.cursor = self.connection.cursor(buffered=True)
            self.cursor.execute(f"USE {self.default_schema}")
            self.cursor.execute(f"SELECT m.id, m.name, d.surname, m.platform, ms.theater_id, ms.time_slot FROM movie m, movie_session ms, director d WHERE m.id = ms.movie_id AND m.director_username = d.username ORDER BY m.id")
            if self.cursor.rowcount == 0:
                return "No movies found"
            result = self.cursor.fetchall()
            for i in range(len(result)):
    ...
    def read_tickets(self):
        try :
            self.cursor = self.connection.cursor(buffered=True)
            self.cursor.execute(f"USE {self.default_schema}")
            self.cursor.execute(f"SELECT t.session_id, t.rating, t.comment FROM ticket t WHERE t.audience_username = '{self.audience}'")
            if self.cursor.rowcount == 0:
                return "No tickets found"
            result = self.cursor.fetchall()
            ticket_list = ["session_id, rating, comment"]
            for row in result:
